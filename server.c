...
void updateProduct(int fd, int newsd, int choice, int fd_admin)
{
    ...
    if (choice == 2 && val == 0)
    {
        deleteProduct(fd, newsd, id, fd_admin);
        return;
    }
    ...
    while (read(fd, &p, sizeof(struct product)))
    {
        if (p.prod_id == id)
        {
            ...
            if (choice == 1)
            {
                ...
                sprintf(log, "Price of product with product id %d updated to %d \n", id, val);
                write(fd_admin, log, strlen(log));
            }
            else
            {
                sprintf(log, "Quantity of product with product id %d updated to %d \n", id, val);
                write(fd_admin, log, strlen(log));
...
void generateAdminLog(int fd_admin, int fd)
{
    ...
    write(fd_admin, "ID\tName\tQuantity\tPrice\n", strlen("ID\tName\tQuantity\tPrice\n"));
...
int main()
{
    ...
    int fd_admin = open("adminLog.txt", O_RDWR | O_CREAT, 0777);
        ...
        if (!fork())
        {
            ...
            else if (login == 2)
            {
                ...
                while (1)
                {
                    ...
                    if (choice == 1)
                    {
                        addProduct(fd_products, newsd, fd_admin);
                    }
                    else if (choice == 2)
                    {
                        updateProduct(fd_products, newsd, 1, fd_admin);
                    }
                    else if (choice == 3)
                    {
                        updateProduct(fd_products, newsd, 2, fd_admin);
                    }
                    else if (choice == 4)
                    {
                        int pid;
                        read(newsd, &pid, 4);
                        deleteProduct(fd_products, newsd, pid, fd_admin);
                    }
                    else if (choice == 5)
                    {
                        showProducts(fd_products, newsd);
                    ...
                    {
                        close(newsd);
                        generateAdminLog(fd_admin, fd_products);
                        break;
                    }
